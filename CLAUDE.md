...
- **Idle electrical baseline ≈ 146W** at the dongle (BINARY rtp during FP-idle on 2026-05-08). The earlier "~60W" figure was the BCD-decoded readout, which is wrong on this unit — see the `ac_total_kwh` column doc above for the BINARY-vs-BCD diagnostic. With the corrected baseline, the lifetime math closes: 146W × ~4500h since 2025-11-01 install = ~657 kWh of idle, which leaves ~960 kWh of compressor work in the 1,617 kWh BINARY lifetime reading — matches expectations for FP-always-on duty through Colorado winter.
...
The original open question on `ac_total_kwh` is **resolved** (BCD vs BINARY format mismatch — see column doc above). One open question generalizes to summer:
