...
- **Turbo (the IR remote's "blast max output" button).** `set_state(turbo=…)` is tri-state (True/False/None); it maps to `dev.turbo` (older msmart-ng: `dev.turbo_mode` — resolved defensively by `_turbo_attr()`, prefers `turbo`). Sent like `freeze_protection`: always send the bit, ignore the unreliable `supports_turbo` flag. **Turbo persists until explicitly cleared** on this Durastar (observed at the hangar 2026-06-15 — it did NOT auto-drop after ~15 min, contrary to an earlier assumption now corrected). It's still deliberately NOT in `_diverged()` and NOT logged per-minute (`state_for_log()` unchanged) — it's a one-shot modifier the UI sets and forgets, so a lingering turbo flag shouldn't raise a spurious "physical remote used" warning; that's a design choice, not a consequence of auto-expiry. Cleared (forced None) in the Freeze Prevention branch — meaningless in FP. Surfaced in `_device_to_dict()` as `turbo` so the Apply checkbox pre-checks and the status line shows it.
