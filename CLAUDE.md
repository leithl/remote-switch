...
- `params TEXT DEFAULT ''` — JSON for HVAC schedules: `{power, mode, target_f, fan_speed, turbo}` or `{mode: "freeze"}`. `turbo` is a bool carrying the full desired state (like the instant Apply form); **absent in pre-turbo schedule rows → `set_state(turbo=None)` → left unchanged** at execute time.
...
The original BCD-vs-BINARY format question on `ac_total_kwh` is **resolved**, but a new W-vs-VA question opened on 2026-05-08 — see the "Caveat" bullet above. One open question remains:
...
- **Purpose: pairing the dongle, then hosting it — and the FlashAir card.** The Midea dongle's pairing app (NetHome Plus) refuses to pair against an open SSID. Many hangar WiFi networks are open. The bridge lets the Pi broadcast its own WPA2 SSID for the dongle to live on, while the Pi stays a client on the open hangar WiFi and NATs the dongle's traffic out.
...
- **Files written by the script:**
  ...
  - `/etc/dnsmasq.d/uap0.conf` — DHCP `192.168.50.50–.150` on `uap0` only (`bind-interfaces` keeps it off `wlan0`), plus static `dhcp-host` reservations below the pool (the FlashAir card at `.20`). Re-runs carry existing reservations over; `FLASHAIR_MAC=` adds or replaces the `.20` one.
...
- **Two clients, both IoT: the Midea dongle and the FlashAir card.** Don't optimize this for many clients. Throughput is irrelevant; reliability over months is what matters.
- **FlashAir card lives on `uap0` (flashair-sync `LINK_MODE=sta`, cut over 2026-09-24).** The card (Toshiba W-03, fw 3.00.01) has `APPMODE=5` in its `/SD_WLAN/CONFIG`, so whenever the avionics power it, it joins `hvac-pair` and gets `192.168.50.20` from the `dhcp-host` reservation on its MAC `ec:21:e5:66:75:62`. flashair-sync probes that IP instead of hopping `wlan0` onto the card's own AP, so SSH/WG/web UI no longer drop during syncs (each hop used to cost ~1.7 min of uplink). Rejoin after power-on takes seconds. Design, cutover record, and **rollback (needs a card re-edit, not just `.env`)**: flashair-sync `docs/appmode5-sta.md`.
  - **Live log, any time the plane is powered in the hangar:** `ssh pi 'f=$(curl -s "http://192.168.50.20/command.cgi?op=100&DIR=/data_log" | tail -1 | cut -d, -f2); curl -s "http://192.168.50.20/data_log/$f"' > live.csv` — read-only snapshot of what the avionics have flushed so far.
  - **Why a flight's last log uploads late:** the card only has power while the avionics are on, and a log only closes at avionics power-off. So the automatic sync picks up a flight's final log at the *next* power-on. To get it uploaded now, cycle the avionics off → on — turning them off alone just makes the card unreachable.
...
- **Don't add a second AP for clients.** This bridge exists for IoT devices (the dongle, the FlashAir card), not to serve users. Adding clients re-introduces all the throughput and channel-sharing issues we accept here.
...
1. **Radio (L1/L2): is the dongle associated to uap0?** `sudo iw dev uap0 station dump`
   - Two stations is normal while the avionics are on — `ec:21:e5:66:75:62` is the FlashAir card; the dongle is `24:59:e5:4c:9d:60`. An idle station can briefly read `tx bitrate: 1.0 MBit/s`; re-check before calling it the UFW trap.
...
- Schedule add: `?sched_dt=YYYY-MM-DDTHH:MM&sched_device=heater|fan|hvac` plus device-specific params (`sched_action=0|1` for heater; `sched_fan_action=on|off|auto` for the fan; `sched_hvac_mode/target_f/fan_speed/power/turbo` for HVAC — `sched_hvac_turbo=1` when checked, absent → off, ignored on the freeze preset).
