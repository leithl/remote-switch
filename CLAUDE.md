...
- **Freeze Prevention is IR-only on this Durastar.** The IR remote's FP button (HEAT + 16°C + ▽▽ within 1s) engages a hidden ~8°C/46°F regulator the LAN protocol can't reach. `MODE_FREEZE` still sets `dev.freeze_protection = True` over LAN (msmart-ng SetState `payload[21]` bit `0x80`), but on this firmware that bit is cosmetic: it shows "FP" on the display and clamps target to 16°C, then the unit heats to ~60°F not ~46°F. We send the bit anyway because some Midea firmwares do honor it (this one doesn't). For real 46°F holding, use the IR remote. Capability flags (`dev.supports_freeze_protection`) are unreliable (msmart-ng issue #76) so we always send the bit regardless. Setting any other explicit `mode` automatically clears the freeze flag — exit FP whenever the user picks a different mode. **`0x0213` is a `CapabilityId` (queried via msg type `0xB5`), not a `PropertyId` (msg type `0xB1`)** — querying it on the property channel returns `size=0` with the error bit, which is expected. Full investigation: `docs/hvac-freeze-prevention-research.md`.
