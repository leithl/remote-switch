...
- **Turbo (the IR remote's "blast max output" button).** `set_state(turbo=…)` is tri-state (True/False/None); it maps to `dev.turbo` (older msmart-ng: `dev.turbo_mode` — resolved defensively by `_turbo_attr()`, prefers `turbo`). Sent like `freeze_protection`: always send the bit, ignore the unreliable `supports_turbo` flag. It's a **transient modifier** — Midea units auto-drop turbo after ~15 min, so it is deliberately NOT in `_diverged()` (would fire spurious "physical remote used" warnings on every self-clear) and NOT logged per-minute (`state_for_log()` unchanged). Cleared (forced None) in the Freeze Prevention branch — meaningless in FP. Surfaced in `_device_to_dict()` as `turbo` so the Apply checkbox pre-checks and the status line shows it.
